...
int sendOffer(struct dhcpMessage *oldpacket)
{
	...
	/* Or the client has a requested ip */
		   ...
		   /*and the ip is not in the pclist table or specific entry in the pclist table*/
			(find_ip_by_mac(oldpacket->chaddr)==NULL||inet_addr(find_ip_by_mac(oldpacket->chaddr))==req_align) &&
	...
	/* otherwise, find a free IP */
		...
		/* check if the mac is in the pclist table,if true then assign the specific ip */
        char * ipaddr=find_ip_by_mac(oldpacket->chaddr);
		if(ipaddr&&!check_ip(inet_addr(ipaddr))){
		 packet.yiaddr = inet_addr(ipaddr);
