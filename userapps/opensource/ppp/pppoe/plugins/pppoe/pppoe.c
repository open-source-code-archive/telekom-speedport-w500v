...
//add by lori for trace ppoe with feaste Ip
int setIpAddress_pppoe( char *cp , int isRemote){
    struct hostent *hp;
    u_int32_t tempIp;

    if ((tempIp = inet_addr( cp )) == (u_int32_t) -1 ) {
        if (( hp = gethostbyname(cp)) == NULL ) {
            option_error("Unknown host: %s",cp);
            //printf("Unknown host: %s",cp);
            return 0;
        }
        tempIp = *(u_int32_t *) hp->h_addr;
    }

    if (bad_ip_adrs(tempIp)) {
        option_error("Bad Ip address: %s",inet_ntoa(tempIp));
        //printf("Bad Ip address: %s",inet_ntoa(tempIp));
        return 0;
    }

    if (tempIp != 0) {
        if ( isRemote == 0) {
            ipcp_wantoptions[0].ouraddr = tempIp;
            //printf("tempIp is %d\n",tempIp);
        } else if ( isRemote == 1 )
            ipcp_wantoptions[0].hisaddr = tempIp;
    }
    return 1;
}
...
void setMtu( char *cp){
    lcp_allowoptions[0].mru = atoi(cp);
}
