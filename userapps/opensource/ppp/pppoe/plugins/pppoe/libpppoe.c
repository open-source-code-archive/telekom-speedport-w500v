...
int session_disconnect_pppoe(struct session *ses){
    ...
    //Ali for dt log
#ifdef LOGMSG_EN
	syslog(LOG_USER, "PPPOE send PADT.\n") ;
#else
    	syslog(LOG_USER, "PPPoE sende PADT\n") ;
#endif
    send_disc(ses,&padt);
...
int session_connect(struct session *ses)
{
	...
	case PADI_CODE:
	{
		...
		//Ali for DT log
#ifdef LOGMSG_EN
		syslog(LOG_USER, "PPPOE send PADI.\n") ;
#else
		syslog(LOG_USER, "PPPoE sende PADI.\n") ;
#endif
	    }
	    break;
	}
	...
	case PADO_CODE:		/* wait for PADO */
	{
		...
		//Ali for DT log
#ifdef LOGMSG_EN
		syslog(LOG_USER, "PPPOE receive PADO.\n") ;
#else
		syslog(LOG_USER, "PPPoE empfange PADO.\n") ;
#endif
	    }
	    break;
	}
	...
	case PADR_CODE:
	{
		...
		//Ali for DT log
#ifdef LOGMSG_EN
		syslog(LOG_USER, "PPPOE send PADR.\n") ;
#else
		syslog(LOG_USER, "PPPoE sende PADR.\n");
#endif
	    }
	    break;
	}
	...
	case PADS_CODE:
	{
		...
		//Ali for DT log
#ifdef LOGMSG_EN
		syslog(LOG_USER, "PPPOE receive PADS.\n") ;
#else
		syslog(LOG_USER, "PPPoE empfange PADS.\n");
#endif
	    }
	    break;
	}
