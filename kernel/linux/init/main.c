...
/*TECOM-20051229-Indy wifi external reset--start*/
...
/*TECOM-20051229-Indy wifi external reset--end*/
...
/*TECOM-20051229-Indy wifi external reset--start*/
//#define __TCM_ENABLE_WIFI_RESET_DEBUG__
#ifdef __TCM_ENABLE_WIFI_RESET_DEBUG__
#define tcmprint		printk
#else
#define tcmprint
#endif

void __init Tcm_resetGPIOs(void)
{
	unsigned long ticks = 0, reset_delay = 0;
	/*
	** 2005.12.28 13:34, wchpeng 
	** add for reset wifi external reset GPIO 31
	** pGPIODir : the r/w control of GPIO 0 ~ 31
	** pGPIOio  : the data of GPIO 0 ~ 31
	*/

	tcmprint( "main.c, %s, org value of GPIO->Dir=0x%08x\n", __func__, (unsigned int)GPIO->GPIODir);
	tcmprint( "main.c, %s, org value of GPIO->Oio=0x%08x\n", __func__, (unsigned int)GPIO->GPIOio);

	/*
	** 2005.12.28 13:36, wchpeng
	** \brief GPIO 31 : wifi external reset
	** \note 0 - trigger to low for 300ms to reset wifi chip - 4318
	*/
	// set GPIO control pin
	*(volatile int*)0xFFFE0404 |= (1<<31);	// wifi external reset
	*(volatile int*)0xFFFE0404 |= (1<<4);	// VoIP1 LED
	*(volatile int*)0xFFFE0404 |= (1<<2);	// VOIP2 LED

	// set GPIO data pin
	*(volatile int*)0xFFFE040C &= ~(1<<31);	// wifi external reset
	*(volatile int*)0xFFFE040C &= ~(1<<4);	// VoIP1 LED
	*(volatile int*)0xFFFE040C &= ~(1<<2);	// VoIP2 LED

	tcmprint( "main.c, %s, \"start - low\" of GPIO->Dir=0x%08x\n", __func__, (unsigned int)GPIO->GPIODir);
	tcmprint( "main.c, %s, \"start - low\" of GPIO->Oio=0x%08x\n", __func__, (unsigned int)GPIO->GPIOio);

	ticks = jiffies;
	reset_delay = ticks + 300;
	tcmprint( "%s, ticks=%u, reset_delay=%u\n", __func__, ticks, reset_delay);

	while ( jiffies < reset_delay)
		;	// nothing, only busy waiting

	ticks = jiffies;
	tcmprint( "%s, ticks=%u\n", __func__, ticks);

	tcmprint( "main.c, %s, \"end - low\" of GPIO->Dir=0x%08x\n", __func__, (unsigned int)GPIO->GPIODir);
	tcmprint( "main.c, %s, \"end - low\" of GPIO->Oio=0x%08x\n", __func__, (unsigned int)GPIO->GPIOio);
	
	*(volatile int*)0xFFFE040C |= (1<<31);
	*(volatile int*)0xFFFE040C |= (1<<4);	
	*(volatile int*)0xFFFE040C |= (1<<2);
	
	tcmprint( "main.c, %s, reset of GPIO->Dir=0x%08x\n", __func__, (unsigned int)GPIO->GPIODir);
	tcmprint( "main.c, %s, reset of GPIO->Oio=0x%08x\n", __func__, (unsigned int)GPIO->GPIOio);	
 ...
 */
	...
	Tcm_resetGPIOs();/*TECOM-20051229-Indy wifi external reset*/
