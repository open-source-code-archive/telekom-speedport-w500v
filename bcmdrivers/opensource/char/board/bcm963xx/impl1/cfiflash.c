...
/* TECOM-20050928-fonchi add to support brust write for mirror bit flash. */
static int flash_burst_write(WORD sector, int offset, byte *buf, int nbytes);
...
/* observed.                                                         */
/*********************************************************************/
/* TECOM-20050928-fonchi modify to support brust write for mirror bit flash. */
int flash_write_buf(WORD sector, int offset, byte *buffer, int numbytes)
{
    ...
    mirrorBitFlash = flash_get_device_id()==ID_AM29LV320MT ? 1:0;
    ...
    for( i = 0; i < 3; i++ ) {
        if( mirrorBitFlash == 1 ) {
            ret = flash_burst_write(sector, offset, buffer, numbytes);
        }
        else {
...
static void flash_command(int command, WORD sector, int offset, UINT16 data)
{
    ...
    case FLASH_AMD:
        ...
        case FLASH_SERASE:
            ...
            flashptr[0] = 0x30;
            break;
        /* TECOM-20050928-fonchi modify to support brust write for mirror bit flash.--start */
        case FLASH_BUFPROG:
            flashptr[0x555] = 0xAA;
            flashptr[0x2AA] = 0x55;
            flashptr[0] = 0x25;
            flashptr[0] = 0x0F;
            break;
        case FLASH_WRITE_BUF: /* TECOM-20050928-fonchi modify to support brust write for mirror bit flash. */
            flashptr[0] = 0x29;
            break;			
        case FLASH_WRITE_BUF_ABORT_RESET: /* TECOM-20050928-fonchi modify to support brust write for mirror bit flash. */
            flashptr[0x555] = 0xAA;
            flashptr[0x2AA] = 0x55;
            flashptr[0x555] = 0xF0;
            break;		
        /* TECOM-20050928-fonchi modify to support brust write for mirror bit flash.--end */
        default:
            break;
        }
        break;
...
static int flash_write(WORD sector, int offset, byte *buf, int nbytes)
{
    ...
    return (int)((int)src - (int)buf);
}

/*********************************************************************/
/* TECOM-20050928-fonchi add to support brust write for mirror bit flash.--start              */
/* flash_burst_write is the fester way to program mirrorBit flash.                               */
/*********************************************************************/
static int flash_burst_write(WORD sector, int offset, byte *buf, int nbytes)
{
	int ret = -1;
	...
	UINT16 *p = (UINT16*)(flash_get_memptr(sector) + offset);
	...
	while( length >= 32 ) {
		flash_command(FLASH_BUFPROG, sector, 0, 0);
			for( i=0; i<(32/2); i++ ) {
			*(pflash + i) = *(pbuf +i);
		}
		flash_command(FLASH_WRITE_BUF, sector, 0, 0);
		if (flash_wait(sector, offset+(nbytes-length), *(pbuf)) != STATUS_READY)
		{
			printk("status fail  length=%d\n",  length );
			break;
		}

		flash_command(FLASH_WRITE_BUF_ABORT_RESET, sector, 0, 0);
		pbuf += 16;
		pflash += 16;
		length -= 32;
	}
	...
	if( length > 0 && length < 32 ) {
		ret = flash_write(sector, (offset + (nbytes-length) ) , (byte*)pbuf, length);
		length -= ret;
	}

	return (int)(nbytes - length);
}
    ...
    if (flashFamily == FLASH_AMD || flashFamily == FLASH_SST) {
        ...
        do {
            ...
            if (d1 == 0) {       /* no toggles, nothing's happening */
                /* 20060404 Dennis Add check data compare Q7 for Mxic suggest */
	        if ( flashptr[offset/2] == data )
                  return STATUS_READY;
            }
        } while (!(/*d1*/ *flashptr & 0x20));
